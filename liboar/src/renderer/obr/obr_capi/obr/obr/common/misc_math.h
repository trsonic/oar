...
inline size_t NextPowTwo(size_t input) {
  ...
  ABSL_DCHECK_LT(static_cast<uint64_t>(input),
...
/*!\brief Unwraps azimuth to (-180, 180] and elevation to [-90, 90] with pole
 * mirroring.
 *
 * \param azimuth_deg Azimuth angle in degrees to unwrap.
 * \param elevation_deg Elevation angle in degrees to unwrap.
 */
inline void UnwrapAzimuthElevation(float& azimuth_deg, float& elevation_deg) {
  // Mirror elevation over the poles and adjust azimuth accordingly.
  while (elevation_deg > 90.0f) {
    elevation_deg = 180.0f - elevation_deg;
    azimuth_deg += 180.0f;
  }
  while (elevation_deg < -90.0f) {
    elevation_deg = -180.0f - elevation_deg;
  ...
  azimuth_deg = std::fmod(azimuth_deg, 360.0f);
  if (azimuth_deg <= -180.0f) {
    azimuth_deg += 360.0f;
  }
  if (azimuth_deg > 180.0f) {
    azimuth_deg -= 360.0f;
