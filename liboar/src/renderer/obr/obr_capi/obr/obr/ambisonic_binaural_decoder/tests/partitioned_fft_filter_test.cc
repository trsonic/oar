...
void ProcessFilterWithImpulseSignal(PartitionedFftFilter* filter,
  ...
  GenerateDiracImpulseFilter(0, &signal_buffer[0]);
  ...
  GenerateSilence(&signal_buffer[0]);
...
TEST(PartitionedFftFilterTest, NonPow2Test) {
  ...
  for (size_t buffer_length = kMinNonPowTwo; buffer_length <= kMaxNonPowTwo;
    ...
    for (size_t filter_length = kLength; filter_length <= 3 * kLength;
      ...
      GenerateIncreasingSignal(&kernel_buffer[0]);
      // Scale from [-1, 1] to desired range.
      for (size_t i = 0; i < filter_length; ++i) {
        kernel_buffer[0][i] =
            (kernel_buffer[0][i] + 1.0f) * filter_length / 8.0f;
      }
...
// length than the original kernel provided at instantiation. It then tests that
...
TEST(PartitionedFftFilterTest, LongerShorterTimeDomainKernelTest) {
  ...
  GenerateIncreasingSignal(&small_kernel_buffer[0]);
  GenerateIncreasingSignal(&big_kernel_buffer[0]);
  // Scale from [-1, 1] to desired range.
  for (size_t i = 0; i < kLength; ++i) {
    small_kernel_buffer[0][i] =
        (small_kernel_buffer[0][i] + 1.0f) * kLength / 8.0f;
  }
  for (size_t i = 0; i < kLength * 2; ++i) {
    big_kernel_buffer[0][i] =
        (big_kernel_buffer[0][i] + 1.0f) * kLength * 2 / 8.0f;
  }
...
TEST(PartitionedFftFilterTest, DiracImpulseTest) {
  ...
  GenerateSawToothSignal(5, &test_signal[0]);
  ...
  GenerateDiracImpulseFilter(kFilterSize / 2, &kernel[0]);
...
class PartitionedFftFilterFrequencyBufferTest : public ::testing::Test {
 protected:
  ...
  void TestFrequencyBufferReset(size_t initial_size, size_t bigger_size,
    ...
    // kernel amounting to 10 partitions we expect just the first 10 buffers to
  ...
  void TestLongerShorterFrequencyDomainKernel(size_t buffer_size) {
    ...
    for (size_t i = 0; i < buffer_size; ++i) {
      small_kernel_buffer[0][i] =
          (small_kernel_buffer[0][i] + 1.0f) * buffer_size / 8.0f;
    }
    for (size_t i = 0; i < 2 * buffer_size; ++i) {
      big_kernel_buffer[0][i] =
          (big_kernel_buffer[0][i] + 1.0f) * buffer_size * 2 / 8.0f;
    }
