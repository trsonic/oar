...
AmbisonicRotator::AmbisonicRotator(int ambisonic_order)
    : ambisonic_order_(ambisonic_order),
      ...
      rotation_matrix_(
          ...
          static_cast<int>(GetNumPeriphonicComponents(ambisonic_order))) {
  ABSL_DCHECK_GE(ambisonic_order_, 1);
...
bool AmbisonicRotator::Process(const WorldRotation& target_rotation,
  ...
  ABSL_DCHECK(output);
  ABSL_DCHECK_EQ(input.num_channels(),
                 GetNumPeriphonicComponents(ambisonic_order_));
  ABSL_DCHECK_EQ(input.num_channels(), output->num_channels());
  ABSL_DCHECK_EQ(input.num_frames(), output->num_frames());
...
void AmbisonicRotator::UpdateRotationMatrix(const WorldRotation& rotation) {
  // Convert from ADM object coordinate system to AmbiX coordinate system.
  // ADM coordinates: X=right, Y=forward, Z=up
  // AmbiX coordinates: X=front, Y=left, Z=up
  WorldRotation ambix_rotation =
      WorldRotation(rotation.w(), -rotation.x(), rotation.z(), rotation.y());

  // Update the rotation sub-matrix for the first order (band 1).
  rotation_matrices_[1] = ambix_rotation.toRotationMatrix();
