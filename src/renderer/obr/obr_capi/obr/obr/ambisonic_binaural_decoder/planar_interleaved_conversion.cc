...
void ConvertInterleavedToPlanarTemplated(
  ...
  ABSL_DCHECK_GE(num_input_frames, input_offset_frames);
  const size_t max_num_input_frames = num_input_frames - input_offset_frames;
  ABSL_DCHECK_GE(num_output_frames, output_offset_frames);
  const size_t max_num_output_frames = num_output_frames - output_offset_frames;
  ABSL_DCHECK_GE(max_num_input_frames, num_frames_to_copy);
  ABSL_DCHECK_GE(max_num_output_frames, num_frames_to_copy);

  if (channel_map == nullptr) {
    ABSL_DCHECK_EQ(num_input_channels, num_output_channels);
  } else {
    ABSL_DCHECK_GE(channel_map->size(), num_output_channels);
  }
  ...
  if (num_input_channels == kNumStereoChannels &&
    ...
    if (channel_map == nullptr) {
      ...
      ABSL_DCHECK_LT((*channel_map)[0], kNumStereoChannels);
      ABSL_DCHECK_LT((*channel_map)[1], kNumStereoChannels);
      DeinterleaveStereo(
    ...
    for (size_t channel_idx = 0; channel_idx < num_output_channels;
      ...
      ABSL_DCHECK_LT(input_channel, num_input_channels);
...
void ConvertPlanarToPlanarTemplated(
  ...
  ABSL_DCHECK_GE(num_output_frames, output_offset_frames);
  const size_t max_num_output_frames = num_output_frames - output_offset_frames;
  ABSL_DCHECK_GE(max_num_input_frames, num_frames_convert_and_copy);
  ABSL_DCHECK_GE(max_num_output_frames, num_frames_convert_and_copy);
...
void ConvertPlanarToInterleavedTemplated(
  ...
  ABSL_DCHECK_NE(interleaved_output_ptr, nullptr);
  ABSL_DCHECK_GE(num_input_frames, input_offset_frames);
  const size_t max_num_input_frames = num_input_frames - input_offset_frames;
  ABSL_DCHECK_GE(num_output_frames, output_offset_frames);
  const size_t max_num_output_frames = num_output_frames - output_offset_frames;
...
void PlanarFromInterleaved(const float* interleaved_buffer,
  ...
  ABSL_DCHECK_NE(interleaved_buffer, nullptr);
  ABSL_DCHECK_GT(planar_buffer_ptr.size(), 0);
...
void FillAudioBuffer(const std::vector<float>& interleaved_buffer,
                     size_t num_input_channels, AudioBuffer* output) {
  ABSL_DCHECK_NE(output, nullptr);
  ABSL_DCHECK_EQ(interleaved_buffer.size() % num_input_channels, 0);
...
void FillAudioBuffer(const float* const* planar_ptrs, size_t num_input_frames,
                     size_t num_input_channels, AudioBuffer* output) {
  ABSL_DCHECK_NE(planar_ptrs, nullptr);
  ABSL_DCHECK_NE(output, nullptr);
  const size_t num_frames_to_copy =
      std::min(num_input_frames, output->num_frames());
...
void FillAudioBufferWithChannelRemapping(const int16_t* const* planar_ptr,
                                         ...
                                         const std::vector<size_t>& channel_map,
                                         AudioBuffer* output) {
  ABSL_DCHECK_NE(planar_ptr, nullptr);
  ABSL_DCHECK_NE(output, nullptr);
  const size_t num_frames_to_copy =
      std::min(num_input_frames, output->num_frames());
  ConvertPlanarToPlanarTemplated<const int16_t* const*, AudioBuffer&>(
...
void GetRawChannelDataPointersFromAudioBuffer(
  ...
  ABSL_DCHECK_NE(audio_buffer, nullptr);
  ABSL_DCHECK_NE(channel_ptr_vector, nullptr);
  ABSL_DCHECK_EQ(audio_buffer->num_channels(), channel_ptr_vector->size());
