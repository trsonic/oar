...
PartitionedFftFilter::PartitionedFftFilter(size_t filter_size,
  ...
  ABSL_CHECK_LE(frames_per_buffer_, chunk_size_);
  ABSL_CHECK_GE(filter_size_, filter_size);
  ABSL_CHECK_GE(max_filter_size_, max_filter_size);
  // Ensure that `filter_size_` does not exceed `max_filter_size_`.
  ABSL_CHECK_LE(filter_size, max_filter_size_);
  // Make sure all partitions have the same size.
  ABSL_CHECK_EQ(num_partitions_ * frames_per_buffer_, filter_size_);
  ABSL_CHECK_EQ(max_num_partitions_ * frames_per_buffer_, max_filter_size_);
...
void PartitionedFftFilter::ResetFreqDomainBuffers(size_t new_filter_size) {
  ...
  ABSL_DCHECK_GT(new_filter_size, 0U);
  filter_size_ =
      CeilToMultipleOfFramesPerBuffer(new_filter_size, frames_per_buffer_);
  ABSL_DCHECK_LE(filter_size_, max_filter_size_);
...
void PartitionedFftFilter::ReplacePartition(
  ...
  ABSL_DCHECK_GE(partition_index, 0U);
  ABSL_DCHECK_LT(partition_index, num_partitions_);
  ABSL_DCHECK_EQ(kernel_chunk.size(), frames_per_buffer_);
...
void PartitionedFftFilter::SetFilterLength(size_t new_filter_size) {
  ...
  new_filter_size =
      CeilToMultipleOfFramesPerBuffer(new_filter_size, frames_per_buffer_);
  ABSL_DCHECK_LE(new_filter_size, max_filter_size_);

  const size_t new_num_partitions = new_filter_size / frames_per_buffer_;
  ABSL_DCHECK_LE(new_num_partitions, max_num_partitions_);
...
void PartitionedFftFilter::SetTimeDomainKernel(
  ...
  for (size_t partition = 0; partition < new_num_partitions; ++partition) {
    ABSL_DCHECK_LE(partition * frames_per_buffer_, kernel.size());
...
void PartitionedFftFilter::SetFreqDomainKernel(const FreqDomainBuffer& kernel) {
  ABSL_DCHECK_LE(kernel.num_channels(), max_num_partitions_);
  ABSL_DCHECK_EQ(kernel.num_frames(), fft_size_);
...
void PartitionedFftFilter::GetFilteredSignal(AudioBuffer::Channel* output) {
  ...
  ABSL_DCHECK_EQ(output->size(), frames_per_buffer_);
