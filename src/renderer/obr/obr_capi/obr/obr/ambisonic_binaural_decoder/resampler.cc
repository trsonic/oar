...
void Resampler::Process(const AudioBuffer& input, AudioBuffer* output) {
  ...
  ABSL_DCHECK_EQ(input.num_channels(), num_channels_);
  const size_t input_length = input.num_frames();
  ABSL_DCHECK_GE(output->num_frames(), GetNextOutputLength(input_length));
  ABSL_DCHECK_LE(output->num_frames(), GetMaxOutputLength(input_length));
  ABSL_DCHECK_EQ(output->num_channels(), num_channels_);
  output->Clear();
  ...
  ABSL_DCHECK_GE(input_sample, input_length);
  ...
  if (samples_left_in_input > 0) {
    ...
    for (size_t channel = 0; channel < num_channels_; ++channel) {
      ...
      ABSL_DCHECK_GT(coeffs_per_phase_, 0U);
      ABSL_DCHECK_GT(input[channel].size(), coeffs_per_phase_ - 1);
...
void Resampler::SetRateAndNumChannels(int source_frequency,
  ...
  ABSL_DCHECK_GT(source_frequency, 0);
  ABSL_DCHECK_GT(destination_frequency, 0);
  ABSL_DCHECK_GT(num_channels, 0U);
...
bool Resampler::AreSampleRatesSupported(int source, int destination) {
  ABSL_DCHECK_GT(source, 0);
  ABSL_DCHECK_GT(destination, 0);
...
void Resampler::InitializeStateBuffer(size_t old_state_num_frames) {
  ...
  if (old_state_num_frames != new_state_num_frames) {
    ...
    for (size_t channel = 0; channel < num_channels_; ++channel) {
      auto& state_channel = state_[channel];
      ABSL_DCHECK_LT(state_channel.begin() + max_size, state_channel.end());
...
void Resampler::GenerateSincFilter(float cutoff_frequency, float sample_rate,
  ...
  ABSL_DCHECK_GT(sample_rate, 0.0f);
  ...
  for (size_t i = 0; i < filter_length; ++i) {
    if (i == half_filter_length) {
      ...
      ABSL_DCHECK_GT(std::abs(denominator), kEpsilonFloat);
      (*buffer_channel)[i] *=
