...
#include "obr/audio_buffer/simd_macros.h"
...
void AddPointwise(size_t length, const float* input_a, const float* input_b,
                  float* output) {
  ABSL_DCHECK_NE(input_a, nullptr);
  ABSL_DCHECK_NE(input_b, nullptr);
...
void MultiplyAndAccumulatePointwise(size_t length, const float* input_a,
  ...
  ABSL_DCHECK_NE(input_a, nullptr);
  ABSL_DCHECK_NE(input_b, nullptr);
  ABSL_DCHECK_NE(accumulator, nullptr);
...
#endif  // SIMD_NEON
  ABSL_DCHECK_EQ(leftover_samples % 2U, 0U);
...
void MonoFromStereoSimd(size_t length, const float* left, const float* right,
                        float* mono) {
  ABSL_DCHECK_NE(left, nullptr);
  ABSL_DCHECK_NE(right, nullptr);
  ABSL_DCHECK_NE(mono, nullptr);
...
void Int16FromFloat(size_t length, const float* input, int16_t* output) {
  ...
  if (output_aligned) {
    ...
    // There is no easy way to simply store the 16 bit ints so we don't have an
...
void FloatFromInt16(size_t length, const int16_t* input, float* output) {
  ...
  if (input_aligned) {
    ...
    // There is no easy way to simply load the 16 bit ints so we don't have an
...
void InterleaveStereo(size_t length, const int16_t* channel_0,
  ...
  ABSL_DCHECK_NE(channel_0, nullptr);
  ABSL_DCHECK_NE(channel_1, nullptr);
